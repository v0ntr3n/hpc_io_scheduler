...
def fit_all(cfg: Config) -> FittedBundle:
    ...
    # DLinear
    dlin, _ = train_dlinear(Xtr, Ytr, cfg, device=cfg_device())

    # Conformal calibration: use a held-out tail of train
    n_calib = min(1000, len(Xtr) // 5)
    Xcal, Ycal = Xtr[-n_calib:], Ytr[-n_calib:]
    m_cal, _ = predict_dlinear(dlin, Xcal, device=cfg_device())
    shp = m_cal.shape
    ...
    return FittedBundle(
        ...
        thresholds=thr, dlinear=dlin, conformal=conformal,
...
def save_bundle(b: FittedBundle, root: str | Path = ARTIFACT_DIR) -> Path:
    ...
    torch.save(b.dlinear.state_dict(), root / "models" / "dlinear_t1.pt")
